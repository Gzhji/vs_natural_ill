...
def build_furn(split_id, house_id, floor_id, pano_id, mesh_id, bsdf_id, position=None,
               texture=None, ground_floor=False, cube_only=False):
...
def add_furns(opt, scene, bsdf_id = 'full', select_name=None, ground_floor=False):
    data_path = f'../data/zind/manualfill/{opt.house}+{opt.floor}+{opt.pano}/'
...
def render_texture(opt, envmap_hdr_path, wall_mesh_dir, bright_rt_wall_mesh_dir,
                      floor_bsdf='white', wall_bsdf='white',
                      ceil_bsdf='white', furn_bsdf='full',
                      floor_tex = None, wall_tex = None, ceil_tex = None,
                      maxd=None, furn=True, samples = 16,
                      wall_extend = True, hem_pos = 0):
    ...
    scene.append(parse_xml('configs/integrators/aov.xml'))
    ...
    scene.append(build_arch(opt, None, None, None, None, 'mask', mesh_dir= wall_mesh_dir, surface_tex = wall_tex,
                            position=np.array([0, 0, 0]), texture=f'{opt.log}/1_win_mask_tex_img.png'))

    if wall_extend:
        scene.append(build_arch(opt, None, None, None, None, 'mask_r', mesh_dir = bright_rt_wall_mesh_dir, surface_tex=wall_tex,
                                position=np.array([0, 0.2, 0]), texture=f'{opt.cache}/3_bright_wall_tex_mask.png'))
